# -*- coding: utf-8 -*-
# STREAMLIT COM MAPA INTERATIVO FOLIUM - VERSÃO COM LAYOUT MELHORADO
...
class DOCXReportGenerator:
    ...
    def build_report(
        ...
        self.add_paragraph(
            f"<b>Período analisado:</b> {analyzer.df['ano'].min()} - {analyzer.df['ano'].max()}"
        )
        ...
        if selections.get("include_metrics"):
            ...
            data_metricas = [
                list(tabela_metricas.columns)
            ] + tabela_metricas.values.tolist()
        ...
        for key, fig in figuras_graficos.items():
            if key == "evolution" and selections.get("include_evolution"):
                ...
                self.add_paragraph(
                    "A análise temporal mostra a evolução dos acidentes ao longo dos anos, permitindo identificar tendências e sazonalidades."
                )
            elif key == "states" and selections.get("include_states"):
                self.add_heading("ANÁLISE POR ESTADO", 2)
                self.add_paragraph(
                    "Comparativo entre estados brasileiros considerando volume de acidentes e taxas de mortalidade."
                )
            elif key == "types" and selections.get("include_types"):
                self.add_heading("TIPOS DE ACIDENTE", 2)
                self.add_paragraph(
                    "Distribuição percentual dos diferentes tipos de acidentes ocorridos no período analisado."
                )
            elif key == "weekday" and selections.get("include_weekday"):
                self.add_heading("PADRÃO SEMANAL DE ACIDENTES", 2)
                self.add_paragraph(
                    "Distribuição dos acidentes por dia da semana, útil para planejamento logístico e operacional."
                )
        ...
        if selections.get("include_highways"):
            ...
            self.add_paragraph(
                "Identificação das rodovias com maior incidência de acidentes e maiores taxas de mortalidade."
            )
            tabela_rodovias = analyzer.create_highways_table()
            data_rodovias = [
                list(tabela_rodovias.columns)
            ] + tabela_rodovias.values.tolist()
        ...
        if selections.get("include_map") or selections.get("include_complete_map"):
            self.add_heading(
                "MAPA DE DISTRIBUIÇÃO GEOGRÁFICA (Visualização na Plataforma)", 2
            )
            self.add_paragraph(
                "Os mapas interativos não podem ser embutidos em documentos Word ou PDF de forma nativa e interativa. Consulte a plataforma para a visualização completa e dinâmica."
            )
        ...
        self.add_paragraph(
            "Este relatório fornece uma visão abrangente da situação dos acidentes rodoviários, identificando padrões, áreas críticas e oportunidades de intervenção para redução de acidentes e melhoria da segurança viária."
        )
...
class DataAnalyzer:
    ...
    def update_font_settings(
        self, title_size=14, label_size=10, legend_size=9, ticks_size=8
    ):
    ...
    def create_evolution_chart(_self):
        ...
        ax1.set_ylabel(
            "Número de Acidentes", fontsize=_self.font_settings["label_size"]
        )
        ax1.tick_params(
            axis="both", which="major", labelsize=_self.font_settings["ticks_size"]
        )
    ...
    def create_states_chart(_self):
        ...
        estados["taxa_mortalidade"] = (
            estados["mortos"] / estados["id"].replace(0, np.nan)
        ) * 100
        ...
        ax1.set_title(
            "Top 10 Estados - Número de Acidentes",
            fontweight="bold",
            fontsize=_self.font_settings["title_size"],
        )
        ax1.set_xlabel(
            "Quantidade de Acidentes", fontsize=_self.font_settings["label_size"]
        )
        ax1.tick_params(
            axis="both", which="major", labelsize=_self.font_settings["ticks_size"]
        )
        ...
        for i, v in enumerate(top10["id"]):
            ax1.text(
                ...
                str(v),
                va="center",
                fontsize=_self.font_settings["ticks_size"],
            )

        # Gráfico 2: Top 10 estados por taxa de mortalidade
        top_mortalidade = estados.nlargest(10, "taxa_mortalidade").dropna(
            subset=["taxa_mortalidade"]
        )
        ax2.barh(
            top_mortalidade["uf"],
        ...
        ax2.set_title(
            "Top 10 Estados - Taxa de Mortalidade",
            fontweight="bold",
            fontsize=_self.font_settings["title_size"],
        )
        ax2.set_xlabel(
            "Taxa de Mortalidade (%)", fontsize=_self.font_settings["label_size"]
        )
        ax2.tick_params(
            axis="both", which="major", labelsize=_self.font_settings["ticks_size"]
        )
    ...
    def create_weekday_chart(_self):
        ...
        df_copy["dia_semana"] = pd.Categorical(
            df_copy["dia_semana"], categories=dias_ordem, ordered=True
        )
        ...
        if semanal.empty:
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.set_title(
                "Padrão Semanal de Acidentes (Sem dados)",
                fontsize=_self.font_settings["title_size"],
            )
            return fig
        ...
        ax.set_title(
            "Padrão Semanal de Acidentes (Acidentes vs. Mortos)",
            fontweight="bold",
            fontsize=_self.font_settings["title_size"],
        )
        ...
        ax.set_ylabel(
            "Número de Acidentes",
            color="#2ecc71",
            fontsize=_self.font_settings["label_size"],
        )
        ax2.set_ylabel(
            "Número de Mortos",
            color="#c0392b",
        ...
        # Ajustar ticks
        ax.tick_params(
            axis="x", rotation=45, labelsize=_self.font_settings["ticks_size"]
        )
        ax.tick_params(
            axis="y", colors="#2ecc71", labelsize=_self.font_settings["ticks_size"]
        )
        ax2.tick_params(
            axis="y", colors="#c0392b", labelsize=_self.font_settings["ticks_size"]
        )
    ...
    def create_interactive_map(_self, df_filtrado, sample_size=1000, map_height=500):
        ...
        for _, estado in estados_data.iterrows():
            ...
            if uf in _self.estados_coords:
                ...
                taxa = (
                    estado["taxa_mortalidade"]
                    if pd.notna(estado["taxa_mortalidade"])
                    else 0
                )
                if taxa > 2:
                    cor = "red"
    ...
    def create_complete_logistics_map(
        _self, df_filtrado, sample_size=1000, map_height=600
    ):  # Adicionado sample_size
        ...
        estados_acidentes["taxa_mortalidade"] = estados_acidentes[
            "taxa_mortalidade"
        ].fillna(
            0
        )  # Tratar NaN
        ...
        for name, config in tiles_config.items():
            folium.TileLayer(config["url"], name=name, attr=config["attr"]).add_to(m2)
        ...
        marker_cluster_estados = plugins.MarkerCluster(
            name="📍 Estados (Agrupados)"
        ).add_to(m2)
        ...
        ranking_mortalidade = estados_acidentes["taxa_mortalidade"].rank(
            method="dense", ascending=False
        )

        # 5. Adicionar Feature Group para Rotas Seguras
        fg_rotas_seguras = folium.FeatureGroup(
            name="✅ Rotas Seguras (Mortalidade Zero)", show=False
        )
        ...
        for _, estado in estados_acidentes.iterrows():
            ...
            if uf in coordenadas_estados:
                ...
                posicao = int(
                    ranking_mortalidade[estados_acidentes["uf"] == uf].iloc[0]
                )
                ...
                folium.Marker(
                    ...
                    tooltip=f"📍 {uf}: {estado['id']} acidentes | {taxa:.1f}% mortalidade",
                    icon=folium.Icon(
                        color=cor_cluster, icon=icon_type_cluster, prefix="fa"
                    ),
                ).add_to(marker_cluster_estados)

                if taxa > 3:
                    icon_color, risco, recomendacao_logistica, cor_recomendacao = (
                        "darkred",
                        "MUITO ALTO",
                        "🚨 EVITAR - Alto risco para operações logísticas",
                        "#ff6b6b",
                    )
                elif taxa > 2:
                    icon_color, risco, recomendacao_logistica, cor_recomendacao = (
                        "red",
                        "ALTO",
                        "⚠️ CUIDADO - Redobrar atenção nas operações",
                        "#ffa726",
                    )
                elif taxa > 1:
                    icon_color, risco, recomendacao_logistica, cor_recomendacao = (
                        "orange",
                        "MÉDIO",
                        "📋 ATENÇÃO - Implementar protocolos de segurança",
                        "#ffd93d",
                    )
                # Marcar Estados com zero mortalidade na camada de Rotas Seguras
                    ...
                    (
                        ...
                        risco_seguro,
                        recomendacao_segura,
                        cor_recomendacao_segura,
                    ) = (
                        "green",
                        "BAIXO",
                        "✅ ADEQUADO - Rotas Seguras (Mortalidade Zero)",
                        "#6bcf7f",
                    )
                    ...
                    folium.Marker(
                        ...
                        icon=folium.Icon(
                            color="green", icon="fa-thumbs-up", prefix="fa"
                        ),
                    ...
                    icon_color, risco, recomendacao_logistica, cor_recomendacao = (
                        "lightgreen",
                        "BAIXO",
                        "✅ ADEQUADO - Condições aceitáveis para logística",
                        "#6bcf7f",
                    )
                ...
                if uf in ["SP", "RJ", "MG", "ES"]:
                    ...
                    contexto_logistica = (
                        "Grandes distâncias - planejar pontos de apoio e combustível"
                    )
                elif uf in ["BA", "PE", "CE", "MA"]:
                    contexto_logistica = (
                        "Condições climáticas variáveis - verificar previsões"
                    )
                else:
                ...
                fg_circulos_risco = folium.FeatureGroup(
                    name="⭕ Estados (Círculos de Risco)", show=True
                ).add_to(m2)
                folium.CircleMarker(
                    location=[lat, lon],
                    radius=10 + (taxa * 3),
        ...
        coords_validas = df_coords[
            (df_coords["lat_clean"].notna()) & (df_coords["lon_clean"].notna())
        ]
        ...
        if coluna_br in df_enriched.columns and coluna_km in df_enriched.columns:
            df_enriched[coluna_br] = (
                df_enriched[coluna_br].fillna("Não informada").astype(str)
            )
            df_enriched[coluna_km] = (
                df_enriched[coluna_km].fillna("Não informado").astype(str)
            )

            acidentes_graves = coords_validas[
                (coords_validas["mortos"] > 0) | (coords_validas["feridos_graves"] > 0)
            ].copy()

            amostra_size = min(200, len(acidentes_graves))
            amostra_acidentes = acidentes_graves.head(amostra_size)

            fg_acidentes_graves = folium.FeatureGroup(
                name="🚨 Acidentes Graves (BR/KM)", show=False
            )
            ...
            for _, acidente in amostra_acidentes.iterrows():
                try:
                    ...
                    br = (
                        str(acidente[coluna_br]).split(".")[0]
                        if "." in str(acidente[coluna_br])
                        else str(acidente[coluna_br])
                    )
                    km = (
                        str(acidente[coluna_km]).split(".")[0]
                        if "." in str(acidente[coluna_km])
                        else str(acidente[coluna_km])
                    )

                    mortos = (
                        int(acidente["mortos"]) if pd.notna(acidente["mortos"]) else 0
                    )
                    feridos_graves = (
                        int(acidente["feridos_graves"])
                        if pd.notna(acidente["feridos_graves"])
                        else 0
                    )

                    if mortos > 0:
                        cor, gravidade, impacto_logistica = (
                            "black",
                            "FATAL",
                            "🔴 PARALISAÇÃO TOTAL - Rota deve ser evitada",
                        )
                    elif feridos_graves > 0:
                        cor, gravidade, impacto_logistica = (
                            "red",
                            "GRAVE",
                            "🟡 ALTO IMPACTO - Considerar rotas alternativas",
                        )
                    else:
                        continue

                    data_acidente = (
                        str(acidente["data_inversa"]).split(" ")[0]
                        if "data_inversa" in acidente
                        and pd.notna(acidente["data_inversa"])
                        else "Data não disponível"
                    )

                    br_principal = any(
                        main_br in br
                        for main_br in ["101", "116", "381", "040", "153", "364", "262"]
                    )
                    contexto_logistica_acidente = (
                        "🚚 ROTA PRINCIPAL - Impacto significativo na logística regional"
                        if br_principal
                        else "🛣️ ROTA SECUNDÁRIA - Impacto localizado"
                    )
        ...
        if coluna_br in df_enriched.columns:
            ...
            rodovias_acidentes["taxa_mortalidade"] = (
                rodovias_acidentes["mortos"] / rodovias_acidentes["id"]
            ) * 100
            rodovias_acidentes["taxa_mortalidade"] = rodovias_acidentes[
                "taxa_mortalidade"
            ].fillna(0)

            rodovias_acidentes["categoria_risco"] = rodovias_acidentes[
                "taxa_mortalidade"
            ].apply(
                lambda x: (
                    "MUITO ALTO"
                    if x > 3
                    else "ALTO" if x > 2 else "MODERADO" if x > 1 else "BAIXO"
                )
            )

            rodovias_acidentes["recomendacao_logistica"] = rodovias_acidentes[
                "categoria_risco"
            ].apply(
                lambda x: (
                    "🚨 EVITAR"
                    if x == "MUITO ALTO"
                        ...
                        "⚠️ CUIDADO EXTREMO"
                        if x == "ALTO"
                        else "📋 ATENÇÃO" if x == "MODERADO" else "✅ ADEQUADO"
                    )
                )
            )

            fg_todas_rodovias = folium.FeatureGroup(
                name="🛣️ Todas as Rodovias (Risco Geral)", show=True
            )

            for idx, rodovia in rodovias_acidentes.iterrows():
                try:
                    br = (
                        str(rodovia[coluna_br]).split(".")[0]
                        if "." in str(rodovia[coluna_br])
                        else str(rodovia[coluna_br])
                    )
                    if br == "Não informada":
                        continue
                    ...
                    if taxa_mortalidade > 3:
                        cor_rodovia, risco_rodovia, cor_bg = (
                            "darkred",
                            "MUITO ALTA",
                            "#ffebee",
                        )
                    ...
                    if len(acidentes_br) > 0:
                        ...
                        logistica_impacto = (
                            "Alto impacto nas operações - buscar alternativas"
                            if taxa_mortalidade > 2
                            else (
                                "Impacto moderado - avaliar custo-benefício"
                                if taxa_mortalidade > 1
                                else "Baixo impacto - condições aceitáveis para logística"
                            )
                        )
                        ...
                        folium.Marker(
                            ...
                            icon=folium.Icon(
                                color=cor_rodovia, icon="fa-road", prefix="fa"
                            ),
        ...
        plugins.MiniMap(tile_layer="CartoDB positron", position="bottomright").add_to(
            m2
        )
    ...
    def create_metrics_table(_self):
        ...
        metrics = {
            ...
            "Taxa Média de Mortalidade": (
                f"{(total_mortos / total_acidentes * 100):.2f}%"
                if total_acidentes > 0
                else "0.00%"
            ),
            "Média de Mortos por Acidente": (
                f"{(total_mortos / total_acidentes):.3f}"
                if total_acidentes > 0
                else "0.000"
            ),
...
# ==============================================================================
# FUNÇÃO PRINCIPAL STREAMLIT - VERSÃO COM LAYOT MELHORADO
# ==============================================================================
def main():
    st.set_page_config(
    ...
    todos_anos = st.sidebar.checkbox(
        "Selecionar Todos os Anos", value=True, key="chk_anos"
    )
    anos_selecionados = (
        anos_disponiveis
        if todos_anos
    ...
    todos_meses = st.sidebar.checkbox(
        "Selecionar Todos os Meses", value=True, key="chk_meses"
    )
    meses_selecionados = (
        meses_disponiveis
        if todos_meses
    ...
    todos_estados = st.sidebar.checkbox(
        "Selecionar Todos os Estados", value=True, key="chk_estados"
    )
    estados_selecionados = (
        list(estados_coords.keys())
        if todos_estados
        else st.sidebar.multiselect(
            "Selecione os estados:",
            options=list(estados_coords.keys()),
            default=list(estados_coords.keys()),
            key="multi_estados",
        )
    )
    ...
    todos_tipos = st.sidebar.checkbox(
        "Selecionar Todos os Tipos", value=True, key="chk_tipos"
    )
    tipos_selecionados = (
        tipos_acidente
        if todos_tipos
        else st.sidebar.multiselect(
            "Selecione os tipos de acidente:",
            options=tipos_acidente,
            default=tipos_acidente,
            key="multi_tipos",
        )
    )
    ...
    if df_filtrado.empty:
        st.warning(
            "⚠️ O refinamento dos filtros resultou em um DataFrame vazio. Ajuste as seleções para continuar."
        )
        return
    ...
    # Slider renomeado para maior clareza
    altura_mapa_completo = st.sidebar.slider("Altura do Mapa (px)", 500, 1000, 650)
    amostra_mapa = st.sidebar.slider("Amostra para Heatmap", 500, 5000, 1500)

    st.sidebar.markdown("---")
    st.sidebar.markdown("### 📄 Conteúdo do Relatório DOCX")

    include_evolution = st.sidebar.checkbox(
        "Incluir Evolução Temporal", value=True, key="rep_evolution"
    )
    include_states = st.sidebar.checkbox(
        "Incluir Análise por Estado", value=True, key="rep_states"
    )
    include_types = st.sidebar.checkbox(
        "Incluir Distribuição por Tipo", value=True, key="rep_types"
    )
    include_weekday = st.sidebar.checkbox(
        "Incluir Padrão Semanal", value=True, key="rep_weekday"
    )
    include_highways = st.sidebar.checkbox(
        "Incluir Ranking de Rodovias", value=True, key="rep_highways"
    )
    include_metrics = st.sidebar.checkbox(
        "Incluir Tabela de Métricas", value=True, key="rep_metrics"
    )
    include_complete_map = st.sidebar.checkbox(
        "Incluir Ref. ao Mapa Completo", value=True, key="rep_complete_map"
    )
    ...
    with tab2:
        ...
        with col_info:
            ...
            st.caption(
                f"Análise de **{len(df_filtrado):,}** registros de **{df_filtrado['ano'].min()}** a **{df_filtrado['ano'].max()}**."
            )
            ...
            st.markdown(
                f'<div class="metric-card">📊 <strong>Total de Acidentes:</strong><br>{total_acidentes:,}</div>',
                unsafe_allow_html=True,
            )
            ...
            with col_met1:
                st.markdown(
                    f'<div class="metric-card">💀 <strong>Mortos:</strong><br>{total_mortos:,}</div>',
                    unsafe_allow_html=True,
                )
            with col_met2:
                st.markdown(
                    f'<div class="metric-card">🏥 <strong>Feridos Graves:</strong><br>{total_feridos_graves:,}</div>',
                    unsafe_allow_html=True,
                )

            st.markdown(
                f'<div class="metric-card">📈 <strong>Taxa de Mortalidade:</strong><br>{taxa_mortalidade:.2f}%</div>',
                unsafe_allow_html=True,
            )
        ...
        with col_rank:
            ...
            if not top_estados.empty:
                ...
                for i, (estado, contagem) in enumerate(top_estados.items()):
                    st.markdown(
                        f'<div class="info-card">{i+1}. <strong>{estado}</strong>: {contagem:,} acidentes</div>',
                        unsafe_allow_html=True,
                    )
            ...
            if not top_rodovias.empty:
                ...
                for i, (rodovia, contagem) in enumerate(top_rodovias.items()):
                    st.markdown(
                        f'<div class="info-card">{i+1}. <strong>{rodovia}</strong>: {contagem:,} acidentes</div>',
                        unsafe_allow_html=True,
                    )
        ...
        with col_carac:
            ...
            if not top_tipos.empty:
                ...
                for i, (tipo, contagem) in enumerate(top_tipos.items()):
                    st.markdown(
                        f'<div class="info-card">{i+1}. <strong>{tipo}</strong></div>',
                        unsafe_allow_html=True,
                    )

            # Dia da semana com mais acidentes
            dia_mais_acidentes = (
                df_filtrado["dia_semana"].value_counts().idxmax()
                if not df_filtrado.empty
                else "N/A"
            )
            total_dia_mais = (
                df_filtrado["dia_semana"].value_counts().max()
                if not df_filtrado.empty
                else 0
            )
            ...
            st.markdown(
                f'<div class="info-card"><strong>{dia_mais_acidentes}</strong><br>{total_dia_mais:,} acidentes</div>',
                unsafe_allow_html=True,
            )
    ...
    with tab4:
        ...
        resumo_executivo = st.text_area(
            ...
            "A análise abrange o período de {} a {}, identificando que o principal desafio de segurança viária está concentrado nas BRs 101, 116 e 040, com ênfase no tipo 'Colisão Traseira' como o mais frequente.".format(
                df_filtrado["ano"].min(), df_filtrado["ano"].max()
            ),
        )
    ...
    st.markdown(
        ...
        """.format(
            df_filtrado["ano"].min(), df_filtrado["ano"].max(), len(df_filtrado)
        ),
        unsafe_allow_html=True,
    )
